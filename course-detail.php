<!doctype html>
<html lang="en">

<?php include 'includes/head.php'  ?> 










  <body class="bg-light GraphikR">

    <!-- --------------------- This is Header ---------------------  -->
     <?php include 'includes/header.php'  ?> 



    <!-- Banner Start -->


        <div class="courseDetailPageBanner goodBackground" style="background-image: url(img/oracle-fusion-technical-banner.jpg);">
          <img src="img/course-detail-page.jpg" alt="" class="img-fluid d-lg-none w-100">
          <div class="container px-3">
            <div class="pt-5"></div>
            <div class="pt-lg-5"></div>

            <div class="row">
              <div class="col-lg-5 pe-lg-4">
                <small class="text-white mb-1">On-Premise Training Program</small>
                <h1 class="text-white AmsiProU text-uppercase">
                  Oracle Fusion Technical
                </h1>
                <small class="text-white mb-1 GraphikM">Oracle Cloud ERP Technical - Integration, Conversion(FBDI, ADFdi, SOAP/REST), Reports(BI/OTBI), Workflows, Security, OIC</small>
                <br>
                <div class="mt-3">
                  <img src="img/four-golden-star.svg" alt="">
                  <small class="text-white">4.7</small>
                  <small class="text-primary ms-2">129 Reviews</small>
                </div>
                <div class="mt-3">
                  <button type="button" class="btn rounded-0 btn-danger py-lg-3 px-lg-4 me-lg-4">Enroll Now <img class="ms-lg-5" src="img/btn-aero.svg" alt=""></button>
                  <button type="button" class="btn rounded-0 text-white py-lg-3 px-lg-4" style=" background-color: #58C1A3; ">Download Syllabus <img  class="ms-2" src="img/btn-aero.svg" alt=""></button>
                </div>
              </div>
            </div>
            <div class="pt-lg-5"></div>
            <div class="pt-5"></div>
          </div>
        </div>


    <!-- Banner End -->









    <!-- About this Training Programs Start -->


        <div class="container px-3">
          <div class="pt-5"></div>
          <div class="pt-3"></div>
          <div class="row">
            <div class="col-lg-5 col-12">
              <h3 class="AmsiProU text-uppercase mb-4">
                About this Training Programs
              </h3>
              <div class="pe-lg-5">
                <small>
                  This Training Programs helps you develop a strong foundation in cloud computing while introducing you to core concepts and features of Oracle Cloud Infrastructure (OCI).
                  <br><br>
                  It also enables you to describe the key features and components of OCI, identify the core OCI service offerings, and understand OCI security model. Plus, you’ll learn the OCI compliance structure and dive deep into OCI pricing, support model, and operations.
                </small>
              </div>
            </div>
            <div class="col-lg-7 col-12">
              <div class="ps-lg-5">
                <div class="row">
                  <div class="col-lg-8 col-12">
                    <div class="row pe-lg-5 pb-5 border-bottom mt-5 mt-lg-0">
                      <div class="col-4">
                        <div class="pe-lg-2 GraphikM">
                          <p>Location</p>
                          <small>On-Premise Gurgaon</small>
                        </div>
                      </div>
                        <div class="col-4">
                          <div class="px-1 GraphikM">
                            <p>Duration</p>
                            <small>3 months @10 hrs/week</small>
                          </div>
                        </div>
                        <div class="col-4">
                          <div class="ps-2 GraphikM">
                            <p>Start Date</p>
                            <small>01 Mar 2023</small>
                          </div>
                        </div>
                      </div>

                      <div class="row pe-lg-5 pt-4">
                        <div class="col-4">
                          <div class="pe-lg-2 GraphikM">
                            <p>Skill level</p>
                            <div class="d-flex align-items-end">
                              <img class="img-fluid me-3 pb-1" src="img/level.svg" alt="">
                              <small class="">Beginner</small>
                            </div>
                          </div>
                        </div>
                        <div class="col-5">
                          <div class="px-1 GraphikM">
                            <p>Range of salary</p>
                            <small>$85–$92k</small>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div class="col-lg-4 col-8 border-start">
                    <div class="ps-lg-4 mt-5 mt-lg-0 GraphikM">
                      <p>Prerequisites</p>
                      <small>
                        <ul>
                          <li>JAVA</li>
                          <li>Basic Understanding of Computer programming</li>
                          <li>MS Office</li>
                          <li>Communication</li>
                        </ul>
                      </small>
                    </div>
                  </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="pt-5"></div>
          <div class="pt-lg-3"></div>

        </div>


    <!-- About this Training Programs Start -->




    <!-- What you will learn Start -->


    <div class="bg-white p-0 m-0">
      <div class="container">
        <div class="pt-5"></div>
        <div class="pt-lg-3"></div>
        <h3 class="AmsiProU text-uppercase">What you will learn</h3>
        <!-- <div class="pt-5"></div> -->
        <!-- <div class="pt-3"></div> -->
        <div class="row">
          <div class="col-lg-5 col-12">
            <div class="ps-2 mainLi">
              <ul class="ps-0 ps-lg-4">
                <li class=""><p class="fs-5">OCI Introduction</p>
                  <div class=" pt-3 pb-2">
                    <p class="text-black-50 border-bottom pb-3 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Get Training on Accounting Hub Cloud Products</p>
                    <p class="text-black-50 border-bottom pb-3 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Job roles Certifications</p>
                    <p class="text-black-50 border-bottom border-dark border-opacity-25 pb-4 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Get Training on Accounting Hub Cloud Products</p>
                  </div>
                </li>
                  
                <li class=""><p class="fs-5">Oracle Cloud Free Tier Account</p>
                  <div class=" pt-3 pb-2">
                    <p class="text-black-50 border-bottom pb-3 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Get Training on Accounting Hub Cloud Products</p>
                    <p class="text-black-50 border-bottom pb-3 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Job roles Certifications</p>
                    <p class="text-black-50 border-bottom border-dark border-opacity-25 pb-4 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Get Training on Accounting Hub Cloud Products</p>
                  </div>
                </li>
                  
                <li class=""><p class="fs-5">Identity & Access Management</p>
                  <div class=" pt-3 pb-2">
                    <p class="text-black-50 border-bottom pb-3 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Get Training on Accounting Hub Cloud Products</p>
                    <p class="text-black-50 border-bottom pb-3 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Job roles Certifications</p>
                    <p class="text-black-50 border-bottom border-dark border-opacity-25 pb-4 d-flex"><img class="me-4" src="img/red-play-button.svg" alt="">Get Training on Accounting Hub Cloud Products</p>
                  </div>
                </li>
                  
              </ul>
            </div>
          </div>
          <div class="col-1 d-none d-lg-block"></div>
          <div class="col-6 d-none d-lg-block">
            <div class="pt-5"></div>
            <div class="pt-5 mt-5"></div>
            <img class="d-none d-lg-block" src="img/ring.svg" alt="">
          </div>
        </div>
      </div>
    </div>


    <!-- What you will learn End -->






    <!-- All our program includes Start -->


      <div class="container-fluid goodBackground" style="background-image: url(img/program-includes-banner.jpg);">
        <div class="pt-5"></div>
        <div class="pt-lg-5"></div>
        <div class="container">
          <h3 class="AmsiProU text-uppercase text-white">All our program includes</h3>
          <div class="pt-lg-5"></div>
          <div class="pt-3"></div>
          <div class="row">
            <div class="col-lg-3 col-12 mt-5 mt-lg-0 text-white">
              <div class="pe-lg-4">
                <img class="pb-lg-4 pb-2" src="img/a-in-white-circle.svg" alt="">
                <p class="GraphikSB fs-5 pb-lg-2">Real-world projects from industry experts</p>
                <small class="GraphikM">With real-world projects and immersive content built in partnership with top-tier companies, you’ll master the tech skills companies want.</small>
              </div>
            </div>
            <div class="col-lg-3 col-12 mt-5 mt-lg-0 text-white">
              <div class="px-lg-2">
                <img class="pb-lg-4 pb-2" src="img/b-in-white-circle.svg" alt="">
                <p class="GraphikSB fs-5 pb-lg-2">Technical mentor support</p>
                <small class="GraphikM">Our knowledgeable mentors guide your learning and are focused on answering your questions, motivating you, and keeping you on track.</small>
              </div>
            </div>
            <div class="col-lg-3 col-12 mt-5 mt-lg-0 text-white">
              <div class="px-lg-2">
                <img class="pb-lg-4 pb-2" src="img/c-in-white-circle.svg" alt="">
                <p class="GraphikSB fs-5 pb-lg-2">Career services</p>
                <small class="GraphikM">You’ll have access to Github portfolio review and LinkedIn profile optimization to help you advance your career and land a high-paying role.</small>
              </div>
            </div>
            <div class="col-lg-3 col-12 mt-5 mt-lg-0 text-white">
              <div class="ps-lg-4">
                <img class="pb-lg-4 pb-2" src="img/d-in-white-circle.svg" alt="">
                <p class="GraphikSB fs-5 pb-lg-2">Flexible learning program</p>
                <small class="GraphikM">Tailor a learning plan that fits your busy life. Learn at your own pace and reach your personal goals on the schedule that works best for you.</small>
              </div>
            </div>
          </div>
        </div>
        <div class="pt-5"></div>
        <div class="pt-lg-5"></div>
        <div class="pt-lg-5"></div>
        <div class="pt-lg-3"></div>
      </div>


    <!-- All our program includes End -->





    <!-- Advantages Start -->


      <div class="bg-dark position-relative">
        <div class="container px-2 overflow-hidden">
          <div class="row">
            <div class="col-5 d-none d-lg-block">
              <img src="img/advantages/men.png" alt="" class="img-fluid">
            </div>
            <div class="col-12 col-lg-7 p-0">
              <div class="bg-danger text-white advantagesBar">
                <div class="pt-lg-5"></div>
                <div class="pt-5"></div>
                <div class="pt-lg-3"></div>
                <div class="px-3 px-lg-5 mx-lg-4">
                  <h2 class="AmsiProU text-uppercase text-white">Advantages</h2>
                  <div class="advantagesLi">
                    <div class="row GraphikSB my-4 pt-lg-5">
                      <div class="col-6">
                        <ul>
                          <li class="">16.5 hours on-demand video</li>
                        </ul>
                      </div>
                      <div class="col-6">
                        <ul>
                          <li class="">Additional suggested resources to improve</li>
                        </ul>
                      </div>
                    </div>

                    <div class="row GraphikSB my-4">
                      <div class="col-6">
                        <ul>
                          <li class="">Practical tips and industry best practices</li>
                        </ul>
                      </div>
                      <div class="col-6">
                        <ul>
                          <li class="">Access on mobile and tv</li>
                        </ul>
                      </div>
                    </div>

                    <div class="row GraphikSB my-4">
                      <div class="col-6">
                        <ul>
                          <li class="">Personalized feedback</li>
                        </ul>
                      </div>
                      <div class="col-6">
                        <ul>
                          <li class="">Get timely feedback on your projects.</li>
                        </ul>
                      </div>
                    </div>

                    <div class="row GraphikSB my-4">
                      <div class="col-6">
                        <ul>
                          <li class="">3 downloadable resources</li>
                        </ul>
                      </div>
                      <div class="col-6">
                        <ul>
                          <li class="">Full lifetime access</li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="pt-lg-5"></div>
                <div class="pt-5"></div>
              </div>
            </div>
          </div>
        </div>
      </div>


    <!-- Advantages End -->





    <!-- Top candidates reviews Start -->


      <div class="container-fluid bg-white">
        <div class="pt-lg-5"></div>
        <div class="pt-5"></div>
        <div class="container px-3">
          <div class="row">
            <div class="col-4 d-none d-lg-block">
              <h3 class="AmsiProU text-uppercase">Top candidates <br> reviews</h3>
            </div>
            <div class="col-lg-8 col-12">
              <div class="">
                <h3 class="AmsiProU text-uppercase d-lg-none mb-5">Top candidates <br> reviews</h3>
                <div class="row">
                  <div class="col-lg-6 col-12">
                    <div class="bg-light px-lg-5 py-lg-5 p-3">
                      <div class="d-flex">
                        <img src="img/top-candidates-reviews/men-icon.png" alt="" class="img-fluid">
                        <div class="ms-4">
                          <p class="GraphikM">Amy R.</p>
                          <div>
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                          </div>
                        </div>
                      </div>
                      <div class="pt-4">
                        <small class="GraphikR">Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse harum ipsum maiores aliquam nihil mollitia corrupti, ab, soluta autem facilis neque qui dolorum quasi labore quibusdam sit, itaque ducimus adipisci repudiandae odio.</small>
                      </div>
                    </div>
                  </div>
                <div class="pt-5 d-lg-none"></div>
                  <div class="col-lg-6 col-12">
                    <div class="bg-light px-lg-5 py-lg-5 p-3">
                      <div class="d-flex">
                        <img src="img/top-candidates-reviews/men-icon.png" alt="" class="img-fluid">
                        <div class="ms-4">
                          <p class="GraphikM">Amy R.</p>
                          <div>
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                          </div>
                        </div>
                      </div>
                      <div class="pt-4">
                        <small class="GraphikR">Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse harum ipsum maiores aliquam nihil mollitia corrupti, ab, soluta autem facilis neque qui dolorum quasi labore quibusdam sit, itaque ducimus adipisci repudiandae odio.</small>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="pt-5"></div>
                <div class="row">
                  <div class="col-lg-6 col-12">
                    <div class="bg-light px-lg-5 py-lg-5 p-3">
                      <div class="d-flex">
                        <img src="img/top-candidates-reviews/men-icon.png" alt="" class="img-fluid">
                        <div class="ms-4">
                          <p class="GraphikM">Amy R.</p>
                          <div>
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                          </div>
                        </div>
                      </div>
                      <div class="pt-4">
                        <small class="GraphikR">Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse harum ipsum maiores aliquam nihil mollitia corrupti, ab, soluta autem facilis neque qui dolorum quasi labore quibusdam sit, itaque ducimus adipisci repudiandae odio.</small>
                      </div>
                    </div>
                  </div>
                <div class="pt-5 d-lg-none"></div>
                  <div class="col-lg-6 col-12">
                    <div class="bg-light px-lg-5 py-lg-5 p-3">
                      <div class="d-flex">
                        <img src="img/top-candidates-reviews/men-icon.png" alt="" class="img-fluid">
                        <div class="ms-4">
                          <p class="GraphikM">Amy R.</p>
                          <div>
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                            <img src="img/top-candidates-reviews/black-star.svg" alt="" class="img-fluid">
                          </div>
                        </div>
                      </div>
                      <div class="pt-4">
                        <small class="GraphikR">Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse harum ipsum maiores aliquam nihil mollitia corrupti, ab, soluta autem facilis neque qui dolorum quasi labore quibusdam sit, itaque ducimus adipisci repudiandae odio.</small>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="pt-5"></div>
        <div class="pt-lg-5"></div>
        <div class="pt-lg-3"></div>
      </div>


    <!-- Top candidates reviews End -->





    <!-- Our Learners Work At Start -->


      <div class="pt-5"></div>
      <div class="pt-5"></div>
      <div class="container position-relative">
        <h3 class="AmsiProU text-uppercase">Our Learners Work At</h3>
      <div class="pt-5"></div>
        <div class="px-5">
          <div class="owl-carousel owl-theme ourLearners">
            <div class="item">
              <div class="bg-white rounded-4 learnersLogoBg">
                <img src="img/our-learners-work-at/1.svg" alt="" class="img-fluid">
              </div>
            </div>
            <div class="item">
              <div class="bg-white rounded-4 learnersLogoBg">
                <img src="img/our-learners-work-at/2.svg" alt="" class="img-fluid">
              </div>
            </div>
            <div class="item">
              <div class="bg-white rounded-4 learnersLogoBg">
                <img src="img/our-learners-work-at/3.svg" alt="" class="img-fluid">
              </div>
            </div>
            <div class="item">
              <div class="bg-white rounded-4 learnersLogoBg">
                <img src="img/our-learners-work-at/4.svg" alt="" class="img-fluid">
              </div>
            </div>
            <div class="item">
              <div class="bg-white rounded-4 learnersLogoBg">
                <img src="img/our-learners-work-at/5.svg" alt="" class="img-fluid">
              </div>
            </div>
            <div class="item">
              <div class="bg-white rounded-4 learnersLogoBg">
                <img src="img/our-learners-work-at/6.svg" alt="" class="img-fluid">
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="pt-5"></div>
      <div class="pt-5"></div>


    <!-- Our Learners Work At End -->





    <!-- Instructor Start  -->

      <div class="container-fluid bg-white">
        <div class="pt-lg-5"></div>
        <div class="pt-5"></div>
        <div class="container px-3">
          <h3 class="AmsiProU text-uppercase">Instructor</h3>
          <div class="pt-5"></div>
          <div class="row">
            <div class="col-1 yellowBorder"></div>
            <div class="col-9">
              <div class="d-lg-flex">
                <img src="img/instructor/1.png" alt="" class="img-fluid">
                <div class="ps-lg-4 pt-4 pt-lg-0">
                  <p class="GraphikM mb-1">Jyoti Sharma</p>
                  <div class="d-flex align-items-center">
                    <img src="img/instructor/black-start.svg" alt="" class="img-fluid me-2">
                    <small class="GraphikM text-muted">4.4 Reviews</small>
                  </div>
                  <small class="GraphikM text-muted">Architect & FreeLancer</small>
                </div>
              </div>
              <div class="pt-lg-4 pt-2">
              <small class="GraphikR text-muted">B.Tech & M.Tech Computer Science at NIT Kurukshetra (Department Ranker 1, CGPA: 9.8) with experience in Android App Development and Machine Learning. I am highly enthusiastic about learning and teaching cutting-edge technologies. My interests include Teaching, Data Structures, Algorithms, and Problem Solving. Worked at Infoedge(Naukri), Ok Google, and got offers from various tech companies including Adobe, UrbanCompany, etc, I would love to create more developers by imparting what I know.</small>
              </div>
            </div>


          </div>

        </div>
        <div class="pt-lg-5"></div>
        <div class="pt-5"></div>
      </div>


    <!-- Instructor End  -->





    <!-- Get Started today Start  -->


        <div class="container-fluid goodBackground position-relative" style="background-image: url(img/get-started-today/banner-bg.jpg);">
          <div class="container">
            <div class="pt-5"></div>
            <div class="pt-lg-5"></div>
            <h3 class="AmsiProU text-uppercase text-white text-center">Get Started today</h3>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-lg-5"></div>
            <div class="pt-4 d-lg-none"></div>
            <div class="d-flex justify-content-center">
              <div class="row priseList">
                <div class="col-1 d-none d-lg-block"></div>
                <div class="col-lg-5 col-12">
                  <div class="p-5 bg-white h-100">
                    <p class="text-black-50 GraphikM mb-0">Monthly access</p>
                    <small class="GraphikSB">Pay as you go</small>
                    <hr>
                    <p class="GraphikSB fs-1 mb-0">₹22849</p>
                    <small class="GraphikSB">per Month</small> <br>
                    <button type="button" class="btn rounded-0 btn-danger py-3 px-4 me-4 mt-4 mb-5">Enroll Now <img class="ms-5" src="img/btn-aero.svg" alt=""></button>

                    <div class="greenTickLi">
                      <ul>
                        <li>Maximum flexibility to learn at your own pace.</li>
                        <li>Cancel anytime.</li>
                      </ul>
                    </div>
                  </div>
                </div>

                <div class="pt-4 d-lg-none"></div>

                <div class="col-lg-5 col-12">
                  <div class="p-5 bg-white h-100">
                    <p class="text-black-50 GraphikM mb-0">4-Monthly access</p>
                    <small class="GraphikSB">Pay upfront and save an extra 15%</small>
                    <hr>
                    <p class="GraphikSB fs-1 mb-0">₹77676</p>
                    <small class="GraphikSB">for 4-Month access</small> <br>
                    <button type="button" class="btn rounded-0 btn-danger py-3 px-4 me-4 mt-4 mb-5">Enroll Now <img class="ms-5" src="img/btn-aero.svg" alt=""></button>

                    <div class="greenTickLi">
                      <ul>
                        <li>Save an extra 15% vs. pay as you go.</li>
                        <li>4 months is the average time to complete this course.</li>
                        <li>Switch to monthly price after if more time is needed.</li>
                        <li>Cancel anytime.</li>
                      </ul>
                    </div>
                  </div>
                </div>
                <div class="col-1 d-none d-lg-block"></div>
              </div>
            </div>
            <div class="pt-5 d-lg-none"></div>
          </div>

        </div>


    <!-- Get Started today End  -->


    <div class="pt-lg-5"></div>
    <div class="pt-lg-5"></div>



        <!-- ---------------------other related Training Programs Start---------------------  -->
      

            <div class="container">
                <div class="mt-5 pt-lg-5"></div>
                  <div class="text-center">
                    <h3 class="text-uppercase text-center AmsiProU">other related Training Programs</h3>
                  </div>
                <div class="mt-4 pt-4"></div>

                <?php include 'includes/related-training-programs.php'  ?>
                
                <div class="mt-5 pt-5"></div>
              </div>

              <div class="container-flued bgLblue">
                <p class="text-white text-center py-4 text-uppercase mb-0">View all Training Programs</p>
            </div>




        <!-- ---------------------other related Training Programs End---------------------  -->



    






















    



    <!-- --------------------- This is Footer ---------------------  -->
        <?php include 'includes/footer.php'  ?>
      
      






        <?php include 'includes/scripts.php'  ?>
  </body>
</html>